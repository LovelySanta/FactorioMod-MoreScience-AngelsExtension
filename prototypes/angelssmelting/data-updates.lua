


--------------------------------------------------------------------------------
----- Angels Smelting                                                      -----
--------------------------------------------------------------------------------

if mods["angelssmelting"] then
  if mods["bobplates"] then
    --BRASS (alloy)
    -- smelting 1:

    -- smelting 2:
    LSlib.technology.addPrerequisite("angels-brass-smelting-2", "angels-metallurgy-3")

    -- smelting 3:
    LSlib.technology.addPrerequisite("angels-brass-smelting-3", "angels-metallurgy-4")



    --BRONZE (alloy)
    -- smelting 1:

    -- smelting 2:

    -- smelting 3:
    LSlib.technology.addPrerequisite("angels-bronze-smelting-3", "angels-metallurgy-3")



    --NITINOL (alloy)
    -- smelting 1:

    -- smelting 2:
    LSlib.technology.addPrerequisite("angels-nitinol-smelting-1", "angels-metallurgy-3")

    -- smelting 3:
  end

--[[
  if mods["bobplates"] then
    --BRASS (alloy)
    -- smelting 1:

    -- smelting 2:
    MoreScience.lib.technology.addPrerequisite("angels-brass-smelting-2", "angels-metallurgy-3")

    -- smelting 3:
    MoreScience.lib.technology.addPrerequisite("angels-brass-smelting-3", "angels-metallurgy-4")



    --BRONZE (alloy)
    -- smelting 1:

    -- smelting 2:

    -- smelting 3:
    MoreScience.lib.technology.addPrerequisite("angels-bronze-smelting-3", "angels-metallurgy-3")



    --NITINOL (alloy)
    -- smelting 1:

    -- smelting 2:
    MoreScience.lib.technology.addPrerequisite("angels-nitinol-smelting-1", "angels-metallurgy-3")

    -- smelting 3:
  end
  ]]
end
